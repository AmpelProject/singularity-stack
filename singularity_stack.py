...
def list_stacks(args):
    ...
    for name, config in stacks.items():
        ...
        for i, service in enumerate(config['services'].keys()):
            ...
            label = _instance_name(name, service, 1)
            try:
            ...
            if active_reps == 0:
                ...
                rep_label = str(active_reps)
            else:
                rep_label = ''
            cols[1:] = service, rep_label, label
...
def logs(args):
    ...
    if args.service is None:
        ...
        if not args.service in config['services']:
            sys.stderr.write("Stack '{}' has no service '{}'. Available services are: {}\n".format(args.name, args.service, ', '.join(config['services'].keys())))
            return 1
        names = [args.service]
